...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="glass-card-solid p-5 mb-8" data-aos="fade-up">
        <form method="GET" action="{{ route('crisis.index') }}" class="flex flex-col sm:flex-row gap-3 items-end">
            <div class="flex-1">
                <label class="block text-xs font-semibold t-muted mb-1">🔍 Find Help in Your Country</label>
            ...
            @if($search)
            <a href="{{ route('crisis.index') }}" class="t-light hover:t-muted text-sm font-medium">Clear</a>
            @endif
        </form>
    ...
    <div class="crisis-banner p-6 mb-8 text-center" data-aos="fade-up">
        <h2 class="font-serif font-bold t-text text-lg mb-2">Need Immediate Help?</h2>
        <p class="t-muted text-sm mb-4">If you're feeling suicidal or are in crisis, please reach out now:</p>
    ...
    <div class="mb-8" data-aos="fade-up">
        <h2 class="font-serif font-bold t-text text-xl mb-4 flex items-center">
            @php
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="glass-card-solid p-5 border-l-4 border-red-300 hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-2">
                    <h3 class="font-bold t-text">{{ $resource->name }}</h3>
                    @if($resource->type)
    ...
    <div class="border-t border-th-border-strong pt-8 mt-4">
        <h2 class="font-serif font-bold t-text text-lg mb-6 text-center">Other Countries</h2>
        @foreach($otherResources as $country => $countryResources)
        <div class="mb-6" data-aos="fade-up">
            <h3 class="font-serif font-bold t-text text-lg mb-3 flex items-center">
                <span class="mr-2">{{ $flags[$country] ?? '🌍' }}</span>{{ $country }}
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @foreach($countryResources as $resource)
                <div class="glass-card-solid p-4 border-l-4 border-red-200">
                    <h4 class="font-bold t-text text-sm mb-1">{{ $resource->name }}</h4>
                    <p class="t-muted text-xs mb-2">{{ $resource->description }}</p>
                    <a href="tel:{{ $resource->phone }}" class="font-bold text-red-500 hover:text-red-600 text-sm">📞 {{ $resource->phone }}</a>
                    @if($resource->url)
                    <span class="t-light mx-2">•</span>
    ...
    <div class="glass-card-solid p-8 mt-8" data-aos="fade-up">
        <h2 class="font-serif font-bold t-text text-xl mb-4 text-center">💚 Safety Planning Tips</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm t-muted">
            <div class="p-4 t-surface rounded-xl">
                <p class="font-semibold t-text mb-2">1. Recognize Warning Signs</p>
                <p>Know your personal triggers and early warning signs of a crisis.</p>
            </div>
            <div class="p-4 t-surface rounded-xl">
                <p class="font-semibold t-text mb-2">2. Use Coping Strategies</p>
                <p>Deep breathing, grounding exercises, and safe activities can help.</p>
            </div>
            <div class="p-4 t-surface rounded-xl">
                <p class="font-semibold t-text mb-2">3. Reach Out to Others</p>
                <p>Contact a trusted friend, family member, or helpline.</p>
            </div>
            <div class="p-4 t-surface rounded-xl">
                <p class="font-semibold t-text mb-2">4. Make Your Environment Safe</p>
